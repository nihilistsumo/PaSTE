...
def get_xy(pair_data, embedding_dir, emb_prefix):
    x = []
    y = []
    paraids = list(np.load(embedding_dir + '/paraids.npy'))
    embed = np.load(embedding_dir + '/' + emb_prefix + '-part1.npy')
    for i in range(len(pair_data)):
        p = pair_data[i]
        p1 = p[0].split("_")[0]
        p2 = p[0].split("_")[1]
        y.append(p[1])
        x.append(np.hstack((embed[paraids.index(p1)], embed[paraids.index(p2)])))
...
def prepare_train_data(emb_prefix, parapair_dict, embedding_dir, hier_qrels_reverse, train_val_split=0.8):
    ...
    Xtrain, ytrain = get_xy(train_pairs, embedding_dir, emb_prefix)
    Xval, yval = get_xy(val_pairs, embedding_dir, emb_prefix)

    return Xtrain, ytrain, Xval, yval

def prepare_test_data(emb_prefix, parapair_dict, embeddings, hier_qrels_reverse, take_random):
    test_pairs = []
    ...
    Xtest, ytest = get_xy(test_pairs, embeddings, emb_prefix)
...
def main():
    ...
    parser.add_argument("-rem", "--train_embedding", required=True, help="Path to train embedding dir")
    parser.add_argument("-sem", "--test_embedding", required=True, help="Path to test embedding dir")
    parser.add_argument("-pre", "--emb_prefix", required=True, help="Common part for each embedding shard file")
    parser.add_argument("-sh", "--sharded_emb", type=bool, default=True, required=True, help="Are the embedding file sharded?")
    ...
    train_embedding_dir = args["train_embedding"]
    test_embedding_dir = args["test_embedding"]
    ...
    Xtrain, ytrain, Xval, yval = prepare_train_data(emb_prefix, train_parapair, train_embedding_dir, hier_qrels_reverse)
    Xtest, ytest = prepare_test_data(emb_prefix, test_parapair, test_embedding_dir, test_hier_qrels_reverse, False)
    Xtest_rand, ytest_rand = prepare_test_data(emb_prefix, test_parapair, test_embedding_dir, test_hier_qrels_reverse, True)
